...
public class SessionManagerWatchdog_dbl : MonoBehaviour
{
	...
	public static void Watch()
	{
		if (forced)
		{
			Debug.Log("[SessionManagerWatchdog_dbl] Watch() skipped: forced already true (stale static from previous Play session - stop Play fully and re-enter, or enable Reload Domain)");
			return;
		}
		if (Object.FindObjectOfType<SessionManagerWatchdog_dbl>() != null)
		{
			Debug.Log("[SessionManagerWatchdog_dbl] Watch() skipped: instance already exists");
			return;
		}
		Debug.Log("[SessionManagerWatchdog_dbl] Watch() creating watchdog GameObject");
	...
	private void Update()
	{
		...
		// ponytail: SessionManager.Awake() sets the singleton before Start() runs, and
		// Start() wipes session back to null. Skip one frame so Start() has definitely
		// run before we touch it.
		if (!seenInstance)
		{
			seenInstance = true;
			return;
		...
		try
		{
			MakeLocalReady(instance);
			forced = true;
		}
		catch (System.Exception e)
		{
			Debug.LogError("[SessionManagerWatchdog_dbl] MakeLocalReady failed, will retry next frame: " + e);
		}
	}

	private void MakeLocalReady(SessionManager instance)
	{
		...
		Debug.Log("[SessionManagerWatchdog_dbl] Session forced fully local - no login, no network");
	}
