...
Shader "N3DS/Effects/Shield (Texture)" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float3 emissive = ((_Color.rgb*lerp( node_5454, (node_5454*-1.0+1.0), _InvertFresnel ))*(_EmissiveIntensity + FastSin(_Time.g * 4) * 0.2));
            ...
            float4 frag(VertexOutput i) : COLOR {
				...
				return i.color + c.a * _IlluminColor;			             
            }
            ENDCG

			//SetTexture[_MainTex]{ combine texture, texture}
			//SetTexture[_MainTex] { ConstantColor[_IlluminColor] combine constant lerp(previous) primary}
        }
    }
}
